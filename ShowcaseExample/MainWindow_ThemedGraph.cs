...
namespace ShowcaseExample
{
    public partial class MainWindow
    {
        ...
        {
            if (tg_Area.AsyncAlgorithmCompute)
        ...
        void tg_deleteitem_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (vc != null)
            {
                foreach (var item in tg_Area.GetRelatedControls(vc, GraphControlType.Edge, EdgesType.All))
                {
                    ...
                    tg_Area.Graph.RemoveEdge(ec.Edge as DataEdge);
                    tg_Area.RemoveEdge(ec.Edge as DataEdge);
                }
                tg_Area.RemoveVertex(vc.Vertex as DataVertex);
                tg_Area.Graph.RemoveVertex(vc.Vertex as DataVertex);
            } 
        }
        ...
        void tg_Area_GenerateGraphFinished(object sender, EventArgs e)
        {
            ...
            tg_zoomctrl.FitToBounds();// ZoomToFill(); //manually update zoom control to fill the area
        ...
        {
            foreach (var item in tg_Area.VertexList.Values)
                HighlightBehaviour.SetHighlightStrategy(item, (HighlightStrategy)tg_highlightStrategy.SelectedItem);
            foreach (var item in tg_Area.EdgesList.Values)
                HighlightBehaviour.SetHighlightStrategy(item, (HighlightStrategy)tg_highlightStrategy.SelectedItem);
        }
        ...
        {
            foreach (var item in tg_Area.VertexList.Values)
                HighlightBehaviour.SetHighlightControl(item, (GraphControlType)tg_highlightType.SelectedItem);
            foreach (var item in tg_Area.EdgesList.Values)
                HighlightBehaviour.SetHighlightControl(item, (GraphControlType)tg_highlightType.SelectedItem);
        }
        ...
        {
            foreach (var item in tg_Area.VertexList.Values)
                HighlightBehaviour.SetIsHighlightEnabled(item, (bool)tg_highlightEnabled.IsChecked);
            foreach (var item in tg_Area.EdgesList.Values)
                HighlightBehaviour.SetIsHighlightEnabled(item, (bool)tg_highlightEnabled.IsChecked);
        ...
        {
            foreach (var item in tg_Area.VertexList.Values)
                HighlightBehaviour.SetHighlightEdges(item, (EdgesType)tg_highlightEdgeType.SelectedItem);
            foreach (var item in tg_Area.EdgesList.Values)
                HighlightBehaviour.SetHighlightEdges(item, (EdgesType)tg_highlightEdgeType.SelectedItem);
        }
